...
export const EditorLayout = ({
  ...
  const computedHeightClasses = useMemo(() => {
    return openDialog
      ? "min-h-[calc(100dvh-52px)] h-[calc(100dvh-52px)]"
      : rowMode
        ? "h-[500px]"
        : "h-[600px]";
  }, [openDialog, rowMode]);
  return (
    ...
    >
      <ResizablePanelGroup
        direction={rowMode ? "horizontal" : "vertical"}
        className={cn(
          "w-full min-h-[500px] max-w-[calc(100dvw)]",
      ...
      >
        ...
        >
          ...
          <Editor
            ...
            className={cn(
              editorClasses,
              "pl-2",
              rowMode ? "pr-1 py-2" : "pr-2 pt-2 pb-1",
            )}
            height={
              openDialog ? (rowMode ? "80dvh" : "37.5dvh") : rowMode ? 500 : 300
            }
        ...
        >
          <div className="absolute bottom-2 right-2 z-30 p-1 rounded-tl-md rounded-br-md">
            <Image src="/max.png" alt="lomi. code" height={30} width={30} />
          </div>
          <Editor
            options={editorOptions}
            className={cn(
              editorClasses,
              "pr-2",
              rowMode ? "pl-1 py-2" : "pl-2 pt-1 pb-2",
            )}
            height={
              openDialog
                ? rowMode
