...
export function UserAuthForm({ className, ...props }: UserAuthFormProps) {
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (isEmailValid && passwordStrength.score >= 4) {
      // Check score >= 4 for minimum strength
      setIsLoading(true);
      setErrorMessage("");
      try {
  ...
  const handleOAuthSignIn = async (provider: "github" | "google") => {
    try {
      ...
      setErrorMessage(
        `An unexpected error occurred while signing in with ${provider}`,
      );
    } finally {
    ...
    <div className={cn("space-y-4 bg-transparent", className)} {...props}>
        ...
        {password && passwordStrength.score < 5 && (
          <div className="mt-2">
            ...
            <div className="flex flex-wrap justify-center gap-1 mt-2">
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-xs ${passwordStrength.isLongEnough ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400"}`}
              >
                8+ chars
              </span>
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-xs ${passwordStrength.hasLower ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400"}`}
              >
                abc
              </span>
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-xs ${passwordStrength.hasUpper ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400"}`}
              >
                ABC
              </span>
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-xs ${passwordStrength.hasNumber ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400"}`}
              >
                123
              </span>
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-xs ${passwordStrength.hasSpecial ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400"}`}
              >
                !@#
              </span>
            </div>
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-xs">
          <span className="px-4 bg-background text-gray-500 dark:text-gray-400">
            Or continue with
          </span>
        </div>
      </div>
