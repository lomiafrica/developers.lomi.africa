...
Lomi uses conventional HTTP status codes to indicate the success or failure of an API request. In general:

-   Codes in the `2xx` range indicate success.
-   Codes in the `4xx` range indicate an error that failed given the information provided (e.g., a required parameter was omitted, a charge failed, etc.).
-   Codes in the `5xx` range indicate an error with Lomi's servers (these are rare).
...
4.  **Implement Logic:** Based on the error type, implement appropriate logic (e.g., prompt user to fix input for `400`, check credentials for `401`, retry later for `429` or `5xx`).
