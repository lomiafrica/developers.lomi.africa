...
2.  **Redirect:** You receive a unique `url` in the response and redirect your customer to this Lomi-hosted page.
3.  **Payment:** The customer selects a payment method and completes the payment on the Lomi page.
...
| `product_id`          | string (UUID)  | No       | Associates the session with a specific Lomi [Product](./products).                                       |
| `subscription_id`     | string (UUID)  | No       | Associates the session with a specific Lomi [Customer Subscription](./customer-subscriptions).             |
| `plan_id`             | string (UUID)  | No       | Associates the session with a specific Lomi [Subscription Plan](./subscriptions).                          |
...
```json
{
  "success_url": "https://your-site.com/order/success?session_id={CHECKOUT_SESSION_ID}",
...
```
*(Note: You can include `{CHECKOUT_SESSION_ID}` in your redirect URLs; Lomi will replace it with the actual session ID before redirecting.)*
