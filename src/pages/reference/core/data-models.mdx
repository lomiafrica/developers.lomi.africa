...
This section describes the structure of common data objects returned by the Lomi API endpoints documented in this core reference.
...
Represents a configured webhook endpoint. Returned by webhook management endpoints (`GET /webhooks`, `POST /webhooks`, `GET /webhooks/{id}`, `PATCH /webhooks/{id}`).

| Property          | Type           | Description                                                                 |
| ----------------- | -------------- | --------------------------------------------------------------------------- |
| `id`              | string         | Unique identifier for the webhook endpoint (prefixed with `wh_`).          |
| `organization_id` | string         | Identifier for the merchant organization this webhook belongs to.           |
| `url`             | string         | The HTTPS URL where webhook events are sent.                                |
| `events`          | array (string) | List of event types this webhook is subscribed to (e.g., `["PAYMENT_SUCCEEDED"]`). See [Webhook Events](./webhooks#webhook-events). |
| `active`          | boolean        | Indicates whether the webhook is currently active and receiving events.     |
| `description`     | string         | Optional description provided for the webhook.                             |
...
**Note:** The webhook `secret` (prefixed with `whsec_`) is **only** returned when creating a webhook (`POST /webhooks`) and is not included when listing or retrieving webhooks.

**Example (from `GET /webhooks/{id}`):**
