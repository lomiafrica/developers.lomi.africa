...
export async function GET() {
  ...
  try {
    ...
    if (authError || !user) {
      console.error("API Route /api/developer-info: Auth error or no user found.", authError);
    ...
    console.log(`API Route /api/developer-info: Authenticated user ID: ${userId}`);
    ...
    if (rpcError) {
      console.error("API Route /api/developer-info: Error calling fetch_developer_ids RPC:", rpcError);
      return NextResponse.json(
    ...
    if (developerData && developerData.length > 0) {
      ...
      const { merchant_id: fetchedMerchantId, organization_id: fetchedOrganizationId } = developerData[0];
      ...
      console.log(`API Route /api/developer-info: Fetched IDs - Merchant: ${merchantId}, Organization: ${organizationId}`);
    } else {
      // Log if no specific data was found, indicating fallback to user ID as merchant ID
      console.warn(`API Route /api/developer-info: No specific developer data found for user ${userId}. Using user ID as merchant ID.`);
    }
