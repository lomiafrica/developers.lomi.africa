  ...
  if (code) {
    ...
    try {
      ...
      if (exchangeError) {
        ...
        return NextResponse.redirect(`${origin}/error?message=Authentication failed: ${encodeURIComponent(exchangeError.message)}`);
      }
      ...
      return NextResponse.redirect(`${origin}/error?message=An unexpected error occurred during authentication.`);
    }
  }
